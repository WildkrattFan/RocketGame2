...
var paused = false
var ranger_base_1_scene = ("res://levels/level1/ranger_base_level1.tscn.tscn")

var name_input_layer
var name_input
var playerEntering = false
var _stateMachine
var jaxson_entered = false
...
func _process(delta):
	if $player/player.can_move == false and State.can_move:
		$player/player.can_move = true
	if State.jaxson_enters and !jaxson_entered:
		jaxsonEnters()
...
func _on_area_2d_area_entered(area: Area2D) -> void:
	if area.name == "playerHitBox":
		get_tree().change_scene_to_file(ranger_base_1_scene)
		print("entering the portal!")


func jaxsonEnters():
	print("jaxson entering!")
	$AnimationPlayer.play("jaxon_enters")
	$Jaxson._stateMachine.travel("flying")
	$player/player.can_move = true
	jaxson_entered = true
